...
class IncoherentDedispersionTask(DedispersionTaskBase):
    ...
    def __init__(self, ih, dm, base_freq, nchan, freq_order=None,
        ...
        nchan_bin = operator.index(nchan) * (1 if ih.complex_data else 2)
        channelized_sample_rate = ih.sample_rate / nchan_bin
        ...
        max_delay_offset = int(np.ceil((
            max_time_delay * channelized_sample_rate).to_value(u.one)))
        ...
        super().__init__(ih, (nsample,) + self._fft.freq_shape[1:],
                         channelized_sample_rate, samples_per_frame,
                         self._fft.freq_dtype)
...
class CoherentDedispersionTask(DedispersionTaskBase):
    ...
    def __init__(self, ih, dm, base_freq, nchan, freq_order=None,
        ...
        self._fft = FFT((self._raw_padframe_len,) + ih.sample_shape,
                        ih.dtype, axis=0, sample_rate=ih.sample_rate)
        self._ifft = self._fft.inverse()

        self.nchan_bin = nchan_bin

        # Channelization FFT.
        if self.nchan_bin > 1:
            self._chan_fft = FFT(((samples_per_frame, nchan_bin) +
                                  ih.sample_shape),
                                 ih.dtype, axis=1, sample_rate=ih.sample_rate)

            super().__init__(ih, (nsample,) + self._chan_fft.freq_shape[1:],
                             channelized_sample_rate, samples_per_frame,
                             self._fft.freq_dtype)
        else:

            super().__init__(ih, (nsample,) + ih.sample_shape,
                             channelized_sample_rate, samples_per_frame,
                             self._fft.freq_dtype)
