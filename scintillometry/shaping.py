...
__all__ = ['Reshape', 'Transpose', 'ReshapeAndTranspose']
...
class SampleShapeChangeBase(TaskBase):
    ...
    def _check_shape(self, value):
        # The values are guaranteed to come from the underlying filehandle so
        # should have been checked already. Here, instead, we apply the
        # operation on the fully broadcast data, and then remove axes in which
        ...
        first_not_unity = next((i for (i, s) in enumerate(value.shape)
                                if s > 1), value.ndim)
        return value.reshape(value.shape[first_not_unity:])
...
class SampleShapeChange(Task, SampleShapeChangeBase):
    ...
    See Also
    --------
    `~scintillometry.shaping.Reshape` : to reshape the samples
    `~scintillometry.shaping.Transpose` : to transpose sample axes
    `~scintillometry.shaping.ReshapeAndTranspose` : to reshape and transpose
...
class Reshape(SampleShapeChangeBase):
    ...
    `~scintillometry.shaping.Transpose` : to transpose sample axes
    `~scintillometry.shaping.ReshapeAndTranspose` : to reshape and transpose
    `~scintillometry.shaping.SampleShapeChange` : to change using a function
...
class ReshapeAndTranspose(Reshape):
    ...
    `~scintillometry.functions.Transpose`, but with fewer intermediate results.
    ...
    See Also
    --------
    `~scintillometry.shaping.Reshape` : to just reshape the samples
    `~scintillometry.shaping.Transpose` : to just transpose sample axes
    `~scintillometry.shaping.SampleShapeChange` : to change using a function
