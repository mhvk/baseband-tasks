...
``PyfftwFFTMaker``, this holds ``flags``, ``threads``, etc.
All such maker classes are registered in the `FFT_MAKER_CLASSES` dict,
keyed by a lower-case version of the name (with ``fftmaker`` removed).
...
__all__ = ['FFTMakerBase', 'FFTBase', 'fft_maker', 'FFT_MAKER_CLASSES']
...
class FFTMakerBase:
    ...
    By subclassing this base, classes are automatically registered in the
    `FFT_MAKER_CLASSES` dict, using a key generated by lowercasing the class's
    name and removing any trailing 'fftmaker' (eg. the key for 'NumpyFFTMaker'
    is 'numpy'). This registration is used by `fft_maker` to select classes.
    """
    ...
    def __init_subclass__(cls, **kwargs):
        key = cls.__name__.lower().removesuffix('fftmaker')
        # Check if class is already registered.
        if key in FFT_MAKER_CLASSES:
            raise ValueError("key {0} already registered in "
                             "FFT_MAKER_CLASSES.".format(key))

        FFT_MAKER_CLASSES[key] = cls
