...
PACKAGENAME = metadata.get('package_name', 'scintillometry')
DESCRIPTION = metadata.get('description', 'A package for radio baseband data reduction and analysis')
AUTHOR = metadata.get('author', 'Marten H. van Kerkwijk, Chenchong Zhu')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://scintillometry.readthedocs.io')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.5")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: scintillometry requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
